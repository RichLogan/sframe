...
TEST_CASE_TEMPLATE_DEFINE("SFrame Test Suite", T, test_suite)
{
    ...
    for (const auto& pair : cases) {
      ...
      for (size_t i = 0; i < tc.epochs.size(); i++) {
        ...
        for (size_t j = 0; j < tc.epochs[i].size(); j++) {
          ...
          std::cout << tc.epochs[i][j] << " " << to_bytes(encrypted)
                    << std::endl;
  ...
  SUBCASE("MLS Known-Answer with Context")
  {
    struct KnownAnswerTest
    {
  ...
  SUBCASE("MLS Round-Trip with context")
  {
    const auto epoch_bits = 4;
    const auto test_epochs = 1 << (epoch_bits + 1);
    const auto epoch_rounds = 10;
  ...
  SUBCASE("MLS Failure after Purge")
  {
