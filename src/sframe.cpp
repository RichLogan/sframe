...
ContextBase::ContextBase(CipherSuite suite_in, provider::ProviderPtr provider)
  : suite(suite_in, validate(std::move(provider)))
{
}

ContextBase::ContextBase(CipherSuiteImpl suite)
  : suite(suite)
{
}
...
{
  if (ciphertext.size() < plaintext.size() + suite.overhead()) {
  ...
  return suite.seal(key_and_salt.key, nonce, ciphertext, aad, plaintext);
}
...
{
  if (ciphertext.size() < suite.overhead()) {
    throw buffer_too_small_error("Ciphertext too small for cipher overhead");
  }

  if (plaintext.size() < ciphertext.size() - suite.overhead()) {
  ...
  const auto aad = header.encoded();
  const auto nonce = form_nonce(header.counter, key_and_salt.salt);
  return suite.open(key_and_salt.key, nonce, plaintext, aad, ciphertext);
}
...
ContextBase::KeyAndSalt::from_base_key(const CipherSuiteImpl& suite, const bytes& base_key)
{
...
Context::Context(CipherSuite suite, provider::ProviderPtr provider)
  : ContextBase(suite, std::move(provider))
{
}
...
MLSContext::MLSContext(CipherSuite suite_in, size_t epoch_bits_in, provider::ProviderPtr provider)
  : Context(suite_in, std::move(provider))
...
MLSContext::MLSContext(CipherSuiteImpl suite_in, size_t epoch_bits_in)
  : Context(suite_in)
  , epoch_bits(epoch_bits_in)
  , epoch_mask((size_t(1) << epoch_bits_in) - 1)
  , epoch_cache(size_t(1) << epoch_bits_in)
...
bytes
MLSContext::EpochKeys::base_key(const CipherSuiteImpl& ciphersuite,
                                SenderID sender_id) const
{
  auto hash_size = ciphersuite.digest_size();
  auto enc_sender_id = bytes(8);
  encode_uint(sender_id, enc_sender_id);

  return ciphersuite.hkdf_expand(sframe_epoch_secret, enc_sender_id, hash_size);
}
