...
{
public:
  ContextBase(CipherSuite suite_in, provider::ProviderPtr provider);
  ContextBase(CipherSuiteImpl suite);
  virtual ~ContextBase();
...
protected:
  struct KeyAndSalt
  {
    static KeyAndSalt from_base_key(const CipherSuiteImpl& suite, const bytes& base_key);
...
private:
  struct EpochKeys
  {
    ...
    bytes base_key(const CipherSuiteImpl& suite, SenderID sender_id) const;
  };
